...
// sshKeepaliveDirectives returns the ServerAlive* directives for the ssh_config.
// With no idle timeout configured, it preserves the long-standing lenient default
// (ServerAliveInterval 180, no ServerAliveCountMax) so local interactive use is unaffected.
// When an idle timeout is configured (e.g. for CI), it derives a tighter keepalive so a
// dead connection is detected within roughly that duration.
func sshKeepaliveDirectives(idleTimeout string) string {
	const defaultDirectives = "ServerAliveInterval 180\n"
	...
	if err != nil {
		logger.Warn("Invalid SSH idle timeout, falling back to default keepalive", "idleTimeout", idleTimeout, "error", err)
		return defaultDirectives
	}

	const probeCount = 3
	interval := int(duration.Seconds()) / probeCount
	if interval < 1 {
		interval = 1
	}

	return fmt.Sprintf("ServerAliveInterval %d\nServerAliveCountMax %d\n", interval, probeCount)
}
...
func GenerateSSHConfigFile(app *config.Atun) (string, error) {
	sshConfigContent := "# SSH over AWS Session Manager (generated by atun.io)\n" +
		"host i-* mi-*\n" +
		sshKeepaliveDirectives(app.Config.SSHIdleTimeout) +
...
// session-manager-plugin only treats SIGINT/SIGTSTP/SIGQUIT as a request to gracefully end the session (it sends a
// TerminateSession flag over the SSM data channel before exiting on those signals). SIGTERM is not handled specially,
// so sending it just kills the process without ending the session server-side, leaving it to age out via AWS's own
// idle timeout exactly like an uncontrolled crash would. Send SIGINT so `atun down` actually closes the SSM session.
